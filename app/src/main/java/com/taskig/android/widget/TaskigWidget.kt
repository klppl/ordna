...
import com.taskig.android.MainActivity
...
import com.taskig.android.ui.theme.appThemeColors
...
/**
 * In-memory reactive data source for the widget.
 * Following the official Android Glance checklist sample pattern:
 * data is exposed as Flows and collected inside provideContent via collectAsState().
 * This ensures recompositions always have fresh data.
 */
object WidgetDataSource {
    private val _tasks = MutableStateFlow<List<TaskEntity>>(emptyList())

    data class SplitTasks(
        val overdue: List<TaskEntity> = emptyList(),
        val today: List<TaskEntity> = emptyList(),
        val completed: List<TaskEntity> = emptyList(),
    )

    fun tasks(): Flow<SplitTasks> {
        val today = LocalDate.now()
        return _tasks.let { flow ->
            combine(flow, MutableStateFlow(today)) { tasks, date ->
                val active = tasks.filter { it.status == "needsAction" && it.due != null && it.due <= date }
                SplitTasks(
                    overdue = active.filter { it.due!! < date },
                    today = active.filter { it.due!! == date },
    ...
    suspend fun reload(context: Context) {
        withContext(Dispatchers.IO) {
            val db = TaskDatabase.getWidgetInstance(context)
            val dao = db.taskDao()
            val today = LocalDate.now()
            val active = dao.getActiveTasks(today)
            val completed = dao.getCompletedTasksList()
            _tasks.value = active + completed
        }
    }

    suspend fun toggleTask(context: Context, taskId: String, listId: String) {
        withContext(Dispatchers.IO) {
            val db = TaskDatabase.getWidgetInstance(context)
            val dao = db.taskDao()
            ...
            // Update in-memory flow immediately for reactive UI update
            _tasks.value = allTasks.map {
                if (it.id == taskId) it.copy(status = newStatus, completedAt = completedAt) else it
            }
...
class TaskigWidget : GlanceAppWidget() {

    override val sizeMode: SizeMode = SizeMode.Exact

    override suspend fun provideGlance(context: Context, id: GlanceId) {
        // Load initial data
        WidgetDataSource.reload(context)
        val initialSplit = WidgetDataSource.tasks().let { flow ->
            val today = LocalDate.now()
            val tasks = (TaskDatabase.getWidgetInstance(context).taskDao().getActiveTasks(today) +
                    TaskDatabase.getWidgetInstance(context).taskDao().getCompletedTasksList())
            WidgetDataSource.SplitTasks(
                overdue = tasks.filter { it.status == "needsAction" && it.due != null && it.due < today },
                today = tasks.filter { it.status == "needsAction" && it.due != null && it.due == today },
                completed = tasks.filter { it.status == "completed" },
            )
        }

        // Load settings
        val settings = SettingsRepository.readWidgetSettings(context)
        ...
        val listOrder = SettingsRepository.readListOrder(context)

        val bg = settings.background
        val opacity = settings.opacity
        val density = settings.layoutDensity
        val showCompleted = settings.showCompleted
        val sorting = settings.sorting
        ...
        provideContent {
            // Collect task data reactively — recomposes when WidgetDataSource emits
            val split by WidgetDataSource.tasks().collectAsState(initial = initialSplit)

            val completedTasks = if (showCompleted) split.completed else emptyList()
            val totalCount = split.overdue.size + split.today.size + completedTasks.size
            val completedCount = completedTasks.size

            GlanceTheme {
                WidgetContent(
                    overdueTasks = split.overdue,
                    todayTasks = split.today,
...
class RefreshWidgetAction : ActionCallback {
    override suspend fun onAction(context: Context, glanceId: GlanceId, parameters: ActionParameters) {
        try {
            com.taskig.android.data.repository.TaskRepository.syncForWidget(context)
        } catch (_: Exception) {
            val workRequest = OneTimeWorkRequestBuilder<com.taskig.android.data.sync.SyncWorker>().build()
            WorkManager.getInstance(context).enqueue(workRequest)
        }
        // Reload the in-memory data source so provideContent recomposes
        WidgetDataSource.reload(context)
    }
}
...
class ToggleTaskAction : ActionCallback {
    override suspend fun onAction(context: Context, glanceId: GlanceId, parameters: ActionParameters) {
        ...
        // Updates Room + in-memory Flow + enqueues API worker
        WidgetDataSource.toggleTask(context, taskId, listId)
    }
}
