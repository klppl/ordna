...
import io.github.klppl.klar.data.repository.WidgetSettings
import io.github.klppl.klar.data.repository.WidgetSorting
import io.github.klppl.klar.ui.theme.AppTheme
import io.github.klppl.klar.ui.theme.appThemeColors
...
class KlarWidget : GlanceAppWidget() {
...
class RefreshWidgetAction : ActionCallback {
    override suspend fun onAction(context: Context, glanceId: GlanceId, parameters: ActionParameters) {
        try {
            ...
            io.github.klppl.klar.data.repository.TaskRepository.syncForWidget(context)
        } catch (_: Exception) {
            // Fall back to async WorkManager sync
            val workRequest = OneTimeWorkRequestBuilder<io.github.klppl.klar.data.sync.SyncWorker>().build()
...
class ToggleTaskAction : ActionCallback {
    override suspend fun onAction(context: Context, glanceId: GlanceId, parameters: ActionParameters) {
        ...
        val isCompleting = task.status == io.github.klppl.klar.data.local.TaskStatus.NEEDS_ACTION
        val newStatus = if (isCompleting) {
            io.github.klppl.klar.data.local.TaskStatus.COMPLETED
        } else {
            io.github.klppl.klar.data.local.TaskStatus.NEEDS_ACTION
        }
